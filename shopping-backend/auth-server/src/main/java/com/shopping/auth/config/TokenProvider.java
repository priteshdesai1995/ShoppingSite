...
public class TokenProvider {
	...
	public String generate(Authentication authentication) {
		...
		return Jwts.builder().header().add("typ", TOKEN_TYPE).and()
				...
				.issuer(TOKEN_ISSUER).audience().add(TOKEN_AUDIENCE).and().subject(user.getUsername())
				.claim("rol", roles).claim("name", user.getName()).claim("userId", user.getId()).claim("preferred_username", user.getUsername())
